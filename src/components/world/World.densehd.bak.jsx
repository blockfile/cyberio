...
const mark = (tx, ty) => occupied.add(`${tx},${ty}`);
const taken = (tx, ty) => occupied.has(`${tx},${ty}`);

for (const [n, tx, ty] of BUILDINGS) { place(n, tx, ty, 0); mark(tx, ty); }

// true if a lot tile touches a road tile (incl. diagonally) — kept clear as a
// sidewalk so the wide HD roads stay fully visible (not buried by buildings).
const nextToRoad = (tx, ty) => {
  for (let dx = -1; dx <= 1; dx++)
    for (let dy = -1; dy <= 1; dy++) {
      if (!dx && !dy) continue;
      if (is(tx + dx, ty + dy, ROADCH)) return true;
    }
  return false;
};

// ---- CROWDED FILL: pack a building onto every free interior lot tile --------
// Walk all lot ('.') tiles on a tight 2-tile lattice and drop a building, so
// blocks read as a dense downtown — but leave the road-edge tiles clear.
const FILL = [...SHORT, ...TALL, ...SHORT];   // mostly shops with towers mixed in
let fi = 0;
for (let ty = 0; ty < GH; ty++)
  for (let tx = 0; tx < GW; tx++) {
    if (cell(tx, ty) !== ".") continue;        // lots only (skip roads/water/park/plaza/dock)
    if ((tx + ty) % 2 !== 0) continue;          // 2-tile lattice so footprints don't fully overlap
    if (taken(tx, ty)) continue;
    if (nextToRoad(tx, ty)) continue;           // keep a sidewalk strip → roads stay visible
    // keep a 1-tile gap from any already-placed building for breathing room
    let near = false;
    for (let dx = -1; dx <= 1 && !near; dx++)
      for (let dy = -1; dy <= 1; dy++) if (taken(tx + dx, ty + dy)) { near = true; break; }
    if (near) continue;
    place(FILL[fi % FILL.length], tx, ty, 0);
    mark(tx, ty);
    fi++;
  }
